...
class NotePad(QtWidgets.QMainWindow):
    ...
    def loadSettings(self):
        ...
        font = self.settings.value("fontText")
        if font and isinstance(font, QFont):
            self.textBox.setFont(font)
        else:
            self.textBox.setFont(QFont("Arial", 10))  # Default font

        # Initialize printers
        self.printer = Qt.QPrinter()
        self.printer1 = Qt.QPrinter()

        # Load page orientation with fallback
        orientation = self.settings.value("orient", Qt.QPrinter.Portrait)  # Default to Portrait
        self.printer1.setOrientation(orientation)

        # Load printer settings with fallbacks
        self.printer.setPrinterName(self.settings.value("printName", ""))  # Default to empty string
        self.printer.setCopyCount(int(self.settings.value("copyCount", 1)))  # Default to 1 copy
